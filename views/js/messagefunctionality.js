...
function sendImg() {
    $.ajax({
        url: `controllers/adminmsghandler.php?msgId=${msg_id}`,
        type: 'POST',
        data: formData,
...
function sendTxt() {
    ...
    if (msg != '') {
        $.ajax({
            url: 'controllers/adminmsghandler.php',
            method: 'POST',
            dataType: 'JSON',
            data: {
                msgTxt: msg,
...
setInterval(() => {
    $.ajax({
        ...
        success: function (data) {
            if ('response' in data) {
                // console.log('in');
                $('.chat-side-content').html(data.response);
            }
        },
    });
}, 1000);
...
$('.chat-side-content').on('click', '.msg-thumb', function () {
    msg_id = $(this).data('msgid');
    $('.chat-heading-username').text(msg_id);
    clicked = true;
});
